...
from app.admin.forms import LoginForm, PwdForm, PreviewFrom, AdminForm, RoleForm, AuthForm, \
    TagForm, MovieForm
from app.models import Role, Admin, Tag, Movie, Oplog, Adminlog, Auth, Userlog, Preview, User, \
    Comment, Moviecol
...
def admin_auth(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        fadmin = Admin.query.filter(
            Admin.id == session["admin_id"]
        ).first()
        supper = fadmin.is_supper
        if (supper == 1):
